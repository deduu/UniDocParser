...
import os
import base64
import requests  # requests only if you want http(s) support
...
class Element(BaseModel):
    ...
    bbox: Optional[List[float]] = Field(
        default=None, serialization_alias="bbox")  # Allow missing
    text: str = ""
...
class Figure(BaseModel):
    ...
    image: Union[Image.Image, str] = Field(
        # reads .pil_image on FigureData
        validation_alias=AliasChoices("image", "pil_image")
    )
    generated_text: str = ""
