...
class ExtractJob(Base):
    ...
    pages = relationship(
        ...
        lazy="selectin",
        passive_deletes=True,     # ← rely on ON DELETE CASCADE
    )
    ...
    __table_args__ = (
        ...
        Index("ix_jobs_tenant_status_created",
...
class ExtractResult(Base):
    ...
    job_id = Column(String, ForeignKey("extract_jobs.id",
                    ondelete="CASCADE"), primary_key=True)
    # Store artifacts in object storage; keep only URLs/keys here
    # DocParserContextOut as JSON file
    ...
    job = relationship("ExtractJob", back_populates="result")
