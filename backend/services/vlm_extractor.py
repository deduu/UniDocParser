...
async def vlm_extract_async(pages: list[dict], pdf_name: str) -> list[dict]:
    """
    ...
    for page in pages:
        idx = page["index"]
        logger.info(
            f"Processing {pdf_name} page {idx} for page {page['image']}")
        ...
        # -------- 2. run VLM extractor ----------------------------------
        output = await formatter.generate_from_image(  # ← async call
            image=pil_image
        )
...
def vlm_extract(pages: list[dict], pdf_name: str) -> list[dict]:
    """
    • Resizes page snapshot to 1080 px width (keeps aspect ratio)
    • Saves the resized image under ./tmp/
    • Calls VLM to generate markdown and stores it back into the page
    """
    return asyncio.run(vlm_extract_async(pages, pdf_name))
